...
class Roborock extends BaseModel
{
    ...
    {
        $output = shell_exec('miiocli roborockvacuum --ip ' . escapeshellcmd($this->settings->roborock->ip->value) . ' --token ' . escapeshellcmd($this->settings->roborock->token->value) . ' ' . $command);
